...
const getTypeProjects = async (req, res) => {
    try {
...
const postTypeProjects = async (req, res) => {
    ...
    try {
        const checkTypeNameQuery = 'SELECT * FROM type_projects WHERE type_name = $1'
        ...
        const result = await pool.query('INSERT INTO type_projects (user_id, type_name) VALUES ($1, $2) RETURNING *', [user_id, type_name]);
        res.status(200).json({
...
const updateTypeProjects = async (req, res) => {
    ...
    try {
        const result = await pool.query(`UPDATE type_projects SET type_name = $1 WHERE type_id = $2 RETURNING *`, [type_name, id]);
        ...
        res.status(200).json({
            ...
            update_typeproject: result.rows[0]
        })
    } catch (err) {
...
module.exports = { getTypeProjects, postTypeProjects, updateTypeProjects }